...
export interface GooeyNavProps {
  ...
  particleR?: number;
  timeVariance?: number;
  colors?: string[]; // We'll now use color strings for particle colors
...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  particleR = 100,
  timeVariance = 300,
  colors = ['#A855F7', '#C084FC', '#8B5CF6'], // purple shades
  ...
  const makeParticles = (element: HTMLElement) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      setTimeout(() => {
        ...
        particle.style.setProperty('--color', p.color); // purple
    ...
    <>
      <style>
        {`
          ...
          .effect.text { color: black !important; }
          .effect.filter { filter: blur(5px); mix-blend-mode: lighten; }
          .effect.filter::before { content: ""; position: absolute; inset: -75px; background: transparent; }
          .particle, .point { display: block; opacity: 0; width: 20px; height: 20px; border-radius: 9999px; transform-origin: center; }
          .particle { position: absolute; top: 50%; left: 50%; animation: particle var(--time) ease 1; }
          .point { background: var(--color); opacity: 1; animation: point var(--time) ease 1; }
          @keyframes particle {
      ...
      <div className="relative" ref={containerRef}>
        <nav className="flex relative">
          <ul ref={navRef} className="flex gap-8 list-none p-0 m-0 relative z-[3] text-black">
            {items.map((item, index) => (
              <li key={index} className="relative cursor-pointer text-black rounded-full">
                <a
                  href={item.href}
